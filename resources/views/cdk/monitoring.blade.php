        ...
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">
                    ...
                    <i></i>TPTKB di Wilayah
                    <span class="badge bg-warning ms-2">{{ $tptkbs->total() }}</span>
                </h5>
                <form method="GET" action="{{ route('cdk.monitoring') }}" class="d-flex">
                    <input type="hidden" name="pbphh_page" value="{{ request('pbphh_page', 1) }}">
                    <!-- <input type="text" class="form-control form-control-sm me-2" name="tptkb_search"
                                    value="{{ request('tptkb_search') }}" placeholder="Cari TPTKB...">
                                <button type="submit" class="btn btn-sm btn-primary">
                                    <i class="fas fa-search"></i>
                                </button> -->
                </form>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @foreach($tptkbs as $tptkb)
                                <tr>
                                    <td>
                                        <strong>{{ $tptkb->tptkb_name }}</strong><br>
                                        <small class="text-muted">{{ $tptkb->nama_pendamping_tptkb ?? 'Belum diisi' }}</small>
                                    </td>
                                    <td>{{ $tptkb->user->email }}</td>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column gap-1">
                                            ...
                                            @if(!$tptkb->is_siap_mitra && !$tptkb->is_siap_tebang)
                                                <span class="badge bg-warning">Belum Siap</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td>
                                        @if($tptkb->materialSupplies && $tptkb->materialSupplies->count() > 0)
                                            @foreach($tptkb->materialSupplies->take(2) as $supply)
                                                <small class="d-block">
                                                    <span class="badge bg-light text-dark">{{ $supply->tipe }}</span>
                                                    {{ $supply->supply_kayu }}
                                                </small>
                                            @endforeach
                                            @if($tptkb->materialSupplies->count() > 2)
                                                <small class="text-muted">+{{ $tptkb->materialSupplies->count() - 2 }} lainnya</small>
                                            @endif
                                        @else
                                            <em class="text-muted">Belum ada data</em>
                                        @endif
                                    ...
                                    <td>
                                        @php
                                            $partnerships = $tptkb->permintaanKerjasama ? $tptkb->permintaanKerjasama->count() : 0;
                                            $active = $tptkb->permintaanKerjasama ? $tptkb->permintaanKerjasama
                                                ->filter(function ($item) {
                                                    return in_array($item->status, ['Disetujui', 'Dijadwalkan', 'Menunggu Tanda Tangan']);
                                                })->count() : 0;
                                        @endphp
        ...
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">
                    ...
                    <i></i>PBPHH di Wilayah
                    <span class="badge bg-primary ms-2">{{ $pbphhs->total() }}</span>
                </h5>
                <form method="GET" action="{{ route('cdk.monitoring') }}" class="d-flex">
